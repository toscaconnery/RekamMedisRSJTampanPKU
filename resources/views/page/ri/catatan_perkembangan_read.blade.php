            ...
            <table class="table table-striped table-advance table-hover">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <div class="btn-group">
                      <a class="btn btn-primary" href="{{url('')}}/ri_catatan_perkembangan_pdf">PDF</a>
              ...
              <table class="table table-bordered">
                ...
                <tbody>
                  ...
                  <tr id="form_{{$p['id_data']}}">
                    <td>
                      <input type="text" autocomplete="off" onkeydown="return false" class="form-control sandbox-container" name="tanggal_{{$p['id_data']}}" disabled value="{{$p['tanggal']}}">
                      <input type="time" class="form-control" name="jam_{{$p['id_data']}}" disabled value="{{$p['jam']}}">
                    </td>
                    <td>
                      <input type="text" class="form-control" name="profesi_{{$p['id_data']}}" readonly value="{{$p['profesi']}}">
                    </td>
                    <td>
                      <textarea style="resize: vertical; width: 100%; -webkit-box-sizing: border-box; -moz-box-sizing: border-box; box-sizing: border-box;" rows="5" class="form-control" name="keterangan_{{$p['id_data']}}" readonly>{{$p['keterangan']}}</textarea>
                    </td>
                    <td><input type="checkbox" class="form-control" name="ttd_{{$p['id_data']}}" {{$p['ttd'] == True ? 'checked' : ''}} disabled></td>
                    <td></td>
                  </tr>
                  @endforeach
                </tbody>
